...
- **Delegation authority follows the canonical action contract.** The discovery metadata, plugin manifests, and skill content describe agent authority as `allowedActions` minus `deniedActions`, with spend bounded by `allowedPaymentTypes` and `maxSpend` and app origins bounded by `allowedOrigins`. The legacy `scope` and `permissions` fields no longer appear in tool descriptions or examples.
...
```bash
npm install @proofable/mcp@0.1.2

...
async function validatePngAssets(pluginDir, pluginName) {
  ...
  for (const file of await walkFiles(assetsDir)) {
    ...
    if (!content.subarray(0, expectedSignature.length).equals(expectedSignature)) {
      addError(`${pluginName}: invalid PNG asset: ${path.relative(repoRoot, file)}`);
    }
  }
}
...
async function main() {
  ...
  for (const [index, entry] of marketplace.plugins.entries()) {
    ...
    await validatePngAssets(pluginDir, entry.name);
